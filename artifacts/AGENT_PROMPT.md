...
**Provided artifacts in the working directory** (read all of them before writing any code):

- `design-retrieval.md` — benchmark formats, retrieval algorithm, expected metric ranges
- `design-storage.md` — S3 blobs + Postgres, async enrichment, unary embedder throughput
- `design-temporal.md` — Temporal Schedule + sweeper workflow replacing the hand-rolled queue
- `design-append-only.md` — immutable ledger, partial unique index invariant, derived views
- `architecture-diagrams.md` — system, pipeline, retrieval, gRPC sequence, run flow
- `TESTING.md` — the five test tiers and what each must assert
- `docker-compose.yml`, `run.sh`, `Makefile` — infrastructure, already written
- `tools/mockembedder/main.go` — deterministic mock embedder, already written
- `internal/store/append_only_test.go` — Postgres invariant tests, already written (helpers are stubs you must implement)
...
**Phase 3 — Pipeline and embedder.** Unicode NFKC normalization, stopword+Snowball tokenization, date extraction (`araddon/dateparse` with `ParseStrict`, `olebedev/when` for relative expressions), round assembly (user+assistant pairs, keeping turn rows for LoCoMo turn-level evidence). Then the `Embedder` interface, the gRPC adapter with the retry service config and keepalive from `design-storage.md` §E.3, and the `embedding_cache` lookup keyed on the **prefixed** text's SHA-256.
...
Watch the test-env timer-skipping gotcha described in `TESTING.md` Tier 1: mock `CountBacklog` to drain, and test the soft-deadline exit separately.
